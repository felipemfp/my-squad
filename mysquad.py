import names, click, math, random, pickle, json
from models import *
...
@click.option('--country', type=click.Choice(mySquad.countries_available()), prompt="Country choice", help="Country that you want to play.")
def new_game(name, country):
    ...
    league_teams = [Team(name, MySquad.generate_squad(18)) for name in league['teams']]
    randomteam = random.choice(league_teams)
    league_teams.remove(randomteam)
    mySquad.current_league = League(league['title'], league_teams + [mySquad.current_team])
    mySquad.save()
...
def league():
    ...
    Show the name of my current team.
    """
    click.echo('Playing {} in {}.'.format(mySquad.current_league.name, mySquad.current_country.capitalize()))
...
def league_details():
    ...
    """
    click.echo('{} in {}:'.format(mySquad.current_league.name, mySquad.current_country.capitalize()))
